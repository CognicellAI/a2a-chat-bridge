...
| Surface Router       | Map DMs and eligible threads to Surfaces; select Contacts and Sessions | `Surface` union type   | Thread ingress is mention-only; channel policy limits allowed Agents; mutations require thread starter or `Manage Threads` ([ADR-0011](../decisions/0011-channel-agent-policies.md)) |
